...
export default function OverviewDynamic({ recipes }: { recipes: any[] }) {
  ...
  // Calculate the last valid page index so last page always shows a full set
  const lastPage = total <= visibleCount ? 0 : Math.ceil((total - visibleCount) / visibleCount) + 1 - 1;
    ...
    <>
      ...
      {recipeOfTheDay && (
        <div style={{ margin: '2rem 0', background: '#fffbe6', borderRadius: '1rem', boxShadow: '0 1px 6px #0001', padding: '1.5rem', display: 'flex', alignItems: 'center', gap: '1.5rem', flexWrap: 'wrap' }}>
          <Image src={recipeOfTheDay.data.images?.[0] || '/logo.png'} alt={recipeOfTheDay.data.title} width={120} height={90} style={{ objectFit: 'cover', borderRadius: '0.75rem' }} />
          <div>
            <div style={{ fontWeight: 700, color: '#b8860b', marginBottom: 4 }}>Recipe of the Day</div>
            <Link href={`/recipes/view/${recipeOfTheDay.slugs?.join('/') || recipeOfTheDay.url}`} style={{ fontSize: '1.25rem', fontWeight: 600, color: '#2d7a46', textDecoration: 'none' }}>{recipeOfTheDay.data.title}</Link>
            <div style={{ color: '#555', marginTop: 4 }}>{recipeOfTheDay.data.description}</div>
          </div>
        </div>
      )}
      ...
      {hasMounted ? (
        <div style={{ position: 'relative', width: '100%', margin: '0 auto 2.5rem', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <div ref={sliderRef} className="keen-slider" style={{ width: '100%', borderRadius: '1rem', height: 340, overflow: 'hidden', flex: 1 }}>
            {featured.map((page, idx) => (
              <div className="keen-slider__slide" key={page.slugs?.join('-') || page.url} style={{ height: 340, display: 'flex', alignItems: 'stretch' }}>
                <Link
                  href={`/recipes/view/${page.slugs?.join('/') || page.url}`}
                  style={{
                    textDecoration: 'none',
                    color: 'inherit',
                    width: '100%',
                  ...
                  onMouseLeave={e => (e.currentTarget.style.boxShadow = '0 2px 12px #0002')}
                >
                  <Image src={page.data.images?.[0] || '/logo.png'} alt={page.data.title} width={320} height={200} style={{ objectFit: 'cover', width: '100%', height: 200, flexShrink: 0 }} />
                  <div style={{ padding: '1.25rem', minHeight: 120, boxSizing: 'border-box', flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'flex-start' }}>
                    <h3 style={{ margin: 0, fontSize: '1.25rem', fontWeight: 600 }}>{page.data.title}</h3>
                    <p style={{ margin: '0.5rem 0 0', color: '#666', flex: 1 }}>{page.data.description}</p>
                  </div>
                </Link>
              </div>
            ))}
      ...
      <div style={{ display: 'flex', gap: '2.5rem', flexWrap: 'wrap', marginBottom: '2rem' }}>
        ...
        <div style={{ background: '#fff', borderRadius: '1rem', boxShadow: '0 1px 6px #0001', padding: '1.5rem 2rem', minWidth: 200, textAlign: 'center' }}>
          <strong style={{ fontSize: '2rem', color: '#2d7a46' }}>{Array.from(new Set(recipes.map(r => r.data.category).filter(Boolean))).length}</strong>
          <div style={{ color: '#555', marginTop: '0.5rem' }}>Categories</div>
        </div>
        <div style={{ background: '#fff', borderRadius: '1rem', boxShadow: '0 1px 6px #0001', padding: '1.5rem 2rem', minWidth: 200, textAlign: 'center' }}>
          <strong style={{ fontSize: '2rem', color: '#2d7a46' }}>{Array.from(new Set(recipes.flatMap(r => r.data.tags || []))).length}</strong>
